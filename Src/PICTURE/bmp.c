...
//返回值:0,成功;
//    其他,错误码.
u8 minibmp_decode2(u8 *filename,u16 x,u16 y,u16 width,u16 height,u16 acolor,uint16_t *data,u8 mode)//尺寸小于240*320的bmp图片解码.
{
	FIL* f_bmp;
    u16 br;
	u8  color_byte;
...
{				
	...
	if((x+width-1)>319)return PIC_WINDOW_ERR;		//区域错误
	if((y+height-1)>239)return PIC_WINDOW_ERR;	//区域错误 
		...
		for(ty=y+height-1;hbmp.bmiHeader.biHeight;ty--)
		{
 			...
 			for(tx=x;pixcnt!=(bi4width/2);)
			{
				if(pixcnt<hbmp.bmiHeader.biWidth)
                    databuf[pixcnt]=0X07FF;//===databuf[pixcnt]=LCD_ReadPoint(tx,ty);//读取坐标点的值 
				else 
...
u8 stdbmp_decode2(const u8 *filename,u16 *data) 
{
	...
	if(res==0)
	{ 
        f_lseek(f_bmp,66);//===sizeof(hbmp) bmp文件头的大小66
        for(u8 i=239;i>0;i--)
        {          
...
{				
 	...
 	if(res==FR_OK)//创建成功
	{
        ...
        ptemp=data; 
        res=f_write(f_bmp,(u8*)ptemp,320*240*2,&bw);//写入数据
