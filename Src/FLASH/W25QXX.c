...
#define SECTOR_SIZE (0X1000)//===4096=4k
//+++
...
u16 W25QXX_ReadID(void)
{
    ...
    // //===test
    // u8 test[14]={0};
    // // QSPI_Send_CMD(W25X_FastReadData,489,8,QSPI_INSTRUCTION_4_LINES,QSPI_ADDRESS_4_LINES,QSPI_ADDRESS_24_BITS,QSPI_DATA_4_LINES);	//QPI,快速读数据,地址为ReadAddr,4线传输数据_32位地址_4线传输地址_4线传输指令,8空周期,NumByteToRead个数据
	// QSPI_Send_CMD(W25X_FastReadData,489,8,QSPI_INSTRUCTION_1_LINE,QSPI_ADDRESS_1_LINE,QSPI_ADDRESS_24_BITS,QSPI_DATA_1_LINE);
    // QSPI_Receive(test,14);
    // //+++
    // W25QXX_Wait_Busy();
    // u8 test[25]={0};
    // SPI_FLASH_BufferRead(test,273,25);
    // W25QXX_Wait_Busy();
    // u8 test2[25]={0};
    // SPI_FLASH_BufferRead(test2,489,25);
    // W25QXX_Wait_Busy();
    // u8 test3[25]={0};
    // SPI_FLASH_BufferRead(test3,0,25);

    // //===
    // W25QXX_Wait_Busy();
    // W25QXX_Erase_Sector(0x3fff10/4096);
    // W25QXX_Wait_Busy();
    // SPI_FLASH_BufferRead(test4,0x3fff10,25);
    // test4[0]=0x11;
    // test4[1]=0x22;
    // test4[2]=0x33;
    // W25QXX_Write_Page(test4,0x3fff10,25);
    // test4[0]=0;
    // test4[1]=0;
    // test4[2]=0;
    // W25QXX_Wait_Busy();
...
//     QSPI_Send_CMD(0x9f,0,0,QSPI_INSTRUCTION_1_LINE,QSPI_ADDRESS_NONE,QSPI_ADDRESS_32_BITS,QSPI_DATA_1_LINE);
	
//     QSPI_Receive(temp,3);
// 	deviceid=(temp[0]<<16)|temp[1]<<8|temp[1];
// 	return deviceid;
// }    
...
{ 
    ...
    QSPI_Send_CMD(0x0b,ReadAddr,8,QSPI_INSTRUCTION_4_LINES,QSPI_ADDRESS_4_LINES,QSPI_ADDRESS_24_BITS,QSPI_DATA_4_LINES);	//QPI,快速读数据,地址为ReadAddr,4线传输数据_32位地址_4线传输地址_4线传输指令,8空周期,NumByteToRead个数据
	QSPI_Receive(pBuffer,NumByteToRead);  
}  
...
void W25QXX_Write_Page(u8* pBuffer,u32 WriteAddr,u16 NumByteToWrite)
{
	...
	QSPI_Send_CMD(W25X_PageProgram,WriteAddr,0,QSPI_INSTRUCTION_4_LINES,QSPI_ADDRESS_4_LINES,QSPI_ADDRESS_24_BITS,QSPI_DATA_4_LINES);	
...
uint8_t BSP_W25Qx_Read_Sector(uint32_t sector_num, uint8_t* pData, uint32_t Size)
{
	...
	if ( left !=0 )
	{
		W25QXX_Read(pBuffer,ReadAddr,left);
	}	
...
uint8_t BSP_W25Qx_Program_Sector(uint32_t sector_num, uint8_t* pData, uint32_t Size)
{
	...
	while(cnt > 0)
	{
		W25QXX_Write(pBuffer,WriteAddr,NumByteToRead); 
		WriteAddr += PAGE_SIZE;
    	pBuffer += PAGE_SIZE;
		cnt--;
	}
	if ( left !=0 )
	{
		W25QXX_Read(pBuffer,WriteAddr,left);
	}	
